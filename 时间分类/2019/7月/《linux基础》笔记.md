...
* /etc/fdprm : 软盘参数表, 用以说明不同的软盘格式, 可用setfdprm的帮助页
* /etc/

#### /dev文件目录

* /dev/console : 系统控制台, 也就是直接和系统连接的监视器。
* /dev/hd : ide硬盘驱动程序接口, 如 : /dev/hda指的是第一个硬盘, had1则是指/dev/hda的第一个分区
* /dev/sd : 磁盘驱动程序接口, 如有系统有scsi硬盘, 就不会访问/dev/had, 而会访问/dev/sda
* /dev/fd : 软驱设备驱动程序, 如: /dev/fd0指系统的第一个软盘, 也就是通常所说的a: 盘, /dev/fd1指第二个软盘以此类推。而/dev/fd1h1440则表示访问驱动器1中的4.5寸高密盘。
* /dev/st : scsi磁带驱动器驱动程序
* /dev/ttv : 提供虚拟控制台支持。 如: /dev/tty1指的是系统的第一个虚拟控制台, /dev/tty2则是系统第二个虚拟控制台
* /dev/ptv : 提供远程登录伪终端支持。 在进行telnet登录时就要用到dev/ptv设备。
* /dev/ttvs : 计算机串行接口
* /dev/cua : 
...
linux采用8进制加法或权限赋予规划了一个很简单的方法来区别读, 写, 执行三种权限 : r用4表示, w用2表示, x用1表示,。
...
``` bash
# 将文件设置为所有人可读 写 执行
[@root ~]$ chmod 777 ./home/xiaobaicai
```

#### chmod特殊权限(SUID)

SUID, 表现在文件所有者的执行权限位上显示为s

#### chattr命令

* 文件可以通过chattr和lsattr来隐藏权限属性的管理。所谓权限属性, 就是默认情况下, 所有新建的文件和目录都没有的, 需要用户根据自己需要精细化设置的权限管理手段。
* chattr : 锁定文件, 不能删除, 不能更改。
* 格式 : chattr [+-=] [ASacdistu] <文件名或目录名>
  * +, - 后面跟一个字母参数, 表明为目标文件增加或减去一个隐藏属性
  * a : 即Append Only, 系统只允许在这个文件之后追加数据, 不允许任何进程覆盖或阶段这个文件。如果目录具有这个属性, 系统将只允许在这个目录下建立和修改文件, 而不允许删除任何文件。

#### ACL和setfacl

ACL即Access Control List, 主要目的是提供传统的拥有者, 用户组, 所有人的读, 写, 执行权限之外的具体权限设置。

如果需要在Linux操作系统中,磁盘或文件系统驱动器使用ACL功能, 首要保证完成两点工作:

* (a) : 设定加载ACL模块配置 : 编辑磁盘列表/etc/fstab配置文件, 指定相应挂载点(目录)加入ACL模块的支持。
* (b) : 重载磁盘设备 : 输入mount -a 命令完成。
